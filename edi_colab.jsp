	...
	<div id="page">
		...
		<div class="wrapper">
				...
				<section class="column width6 first">					
					...
					<form id="sampleform" method="post" action="edi_colab_do.jsp">
                                            
                                             <%
                              String id = request.getParameter("id"); 
                              Class.forName("com.mysql.jdbc.Driver");
                              Connection conn = DriverManager.getConnection("jdbc:mySQL://localhost:3306/projeto2?useUnicode=true","root","admin");
                              PreparedStatement ps = conn.prepareStatement("SELECT  CL_ID, CL_LOGIN, CL_PASS, CL_NOME, CL_NIVEL, CL_EMAIL, nv_nome FROM Colaboradores inner join nivel on cl_nivel = nv_id Where cl_id = " +id);
                              //CL_ID, CL_LOGIN, CL_PASS, CL_NOME, CL_NIVEL, CL_EMAIL
                              //id nome, login, pass, nivel, email
...
<%        while(rs.next()){   %>
							<p>
								<label class="required" for="nome">Nome:</label><br/>
                                                                <input type="hidden" id="id" class="half" value="<%=rs.getString("cl_id")%>" name="id"/>
								<input type="text" id="nome" class="half" value="<%=rs.getString("cl_nome")%>" name="nome"/>
							</p>
                                                        
                                                        <p>
								<label class="required" for="login">Login:</label><br/>
								<input type="text" id="login" class="half" value="<%=rs.getString("cl_login")%>" name="login"/>
							</p>
                                                        
                                                         <p>
								<label class="required" for="pass">Password:</label><br/>
								<input type="password" id="pass" class="half" value="<%=rs.getString("cl_pass")%>" name="pass"/>
							</p>
								...
								<label class="required" for="nivel">Nivel</label><br/>
								  <select name="nivel" id="nivel">
                                                                      <option value="<%=rs.getString("cl_nivel")%>"><%=rs.getString("nv_nome")%></option>
							...
							<p>
								...
								<input type="text" id="email" class="half" value="<%=rs.getString("cl_email")%>" name="email"/>
							</p>
